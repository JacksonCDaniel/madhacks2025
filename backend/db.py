...
def create_conversation(system_message, user_id=None, metadata=None):
    conn = _connect()
    try:
        cur = conn.cursor()
        ...
        cur.execute(
            "INSERT INTO conversations (id, created_at, updated_at, system_message, user_id, metadata) VALUES (?, ?, ?, ?, ?, ?)",
            (conv_id, now, now, system_message, user_id, meta_text),
        )
        conn.commit()
        return conv_id
    finally:
