...
export default function InterviewOverlay({ company, voice, topic, onEnd }) {
  ...
  const loadMessages = async (convId) => {
    try {
      const res = await fetch(`${API_BASE}/conversations/${convId}/messages?limit=1000`);
      if (!res.ok) {
              ...
              {messages.map((msg) => (
                <div key={msg.id} className={`message ${msg.role}`}>
                  ...
                  <div className="message-content">
                    <div>{msg.content || "..."}</div>
                    {msg.audioUrl && <audio controls autoPlay src={msg.audioUrl}></audio>}
                  </div>
                </div>
              ))}
              {isTyping && (
                <div className="message assistant">
                  ...
                  <div className="message-content">
                    <div style={{ display: "flex", gap: "4px" }}>
                      <span className="typing-dot" style={{ width: 8, height: 8, background: "#999", borderRadius: "50%", animation: "typing 1.4s infinite" }}></span>
                      <span className="typing-dot" style={{ width: 8, height: 8, background: "#999", borderRadius: "50%", animation: "typing 1.4s infinite 0.2s" }}></span>
                      <span className="typing-dot" style={{ width: 8, height: 8, background: "#999", borderRadius: "50%", animation: "typing 1.4s infinite 0.4s" }}></span>
                    </div>
                  </div>
                </div>
              )}
